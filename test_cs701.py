...
def inference(args, model, test_save_path=None):
    """
    Perform inference on test dataset using the trained model.

    Args:
        args: Parsed command line arguments
        model: Trained neural network model
        test_save_path: Path to save test predictions (optional)

    Returns:
        str: Confirmation message after testing completion
    """
    # Initialize test dataset and dataloader
    db_test = Synapse_dataset(
        base_dir=args.volume_path, split=args.split_name, list_dir=args.list_dir
    )
    ...
    for i_batch, sampled_batch in tqdm(enumerate(testloader)):
        ...
        # Handle dataset-specific case name formatting
        if args.dataset == "datasets":
            case_name = split(case_name.split(",")[0])[-1]

        # Perform inference on single volume
...
if __name__ == "__main__":
    # Set up deterministic behavior if specified
    ...
    dataset_config = {
        "cs701": {
            "root_path": "./datasets/cs701_224/test_vol_h5",
    ...
    # Update arguments based on dataset configuration
    ...
    # Set up test save directory if needed
    if args.is_savenii:
