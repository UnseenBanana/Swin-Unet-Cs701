...
def preprocess_val_image(image_files: str, label_files: str) -> None:
    os.makedirs(f"{args.dst_path}/val_npz", exist_ok=True)
    ...
    for image_file, label_file in pbar:
        ...
        save_path = f"{args.dst_path}/val_npz/case{case_num}_slice{slice_num}.npz"
        np.savez(save_path, label=label_data, image=image_data)
    pbar.close()


def preprocess_test_image(image_files: list, label_files: list) -> None:
...
if __name__ == "__main__":
    ...
    preprocess_val_image(image_files, label_files)
    preprocess_test_image(image_files, label_files)
